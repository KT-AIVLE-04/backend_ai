...
def generate_korean_markdown_report(report_data: dict, classified_industry: str, analysis_data: PostAnalysisRequest) -> str:
    ...
    시각화 표현 규칙:
    - 프로그레스바나 그래프를 표현할 때는 적절한 크기로 제한하세요.
    - 전체 페이지(가로/세로)를 10타일로 가정할 때, 최대 6타일까지만 사용하세요.
    - 긴 프로그레스바(████████████)보다는 적당한 길이(██████)를 사용하세요.
    - 표는 가로 스크롤이 발생하지 않도록 적절한 너비로 조정하세요.
    - 막대 그래프나 차트는 컴팩트하고 읽기 쉽게 표현하세요.
    """
    ...
    9. 시각적 요소(프로그레스바, 표)는 적절한 크기로 제한하여 전체 페이지 레이아웃을 깨트리지 않도록 유지
    10. 막대 그래프나 차트 표현 시 컴팩트한 형태 사용 (예: ███░░ 60% 형태)
